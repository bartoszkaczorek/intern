define([
	...
	'dojo/node!fs',
	'dojo/aspect',
	'dojo/node!./factory/winstonFactory',
	'./util'
], function (require, lang, Promise, http, path, fs, aspect, winstonFactory, util) {
	/* jshint node:true */

	function getLoggerOptions(config) {
		var result =  {};
		if (config !== null && config !== undefined && config.internServerOptions !== null && config.internServerOptions !== undefined && config.internServerOptions.logger != null && config.internServerOptions.logger !== undefined) {
			result = config.internServerOptions.logger;			
		}
		return result;
	}
	...
	function Proxy(config) {
		...
		var loggerOptions = getLoggerOptions(config);
		this.logger = winstonFactory.createLogger(loggerOptions);
			...
			return new Promise(function (resolve) {
				...
				server.on('request', function(request, response){
					...
					response.on('finish', function(){
						...
						logger.info('Request finished: %s %s %s [%sms]', response.statusCode, request.method, request.url, end);
					});
				});
